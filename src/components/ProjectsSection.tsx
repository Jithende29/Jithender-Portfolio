    ...
    <section id="projects" className="scroll-mt-28 rounded-[40px] border border-white/10 bg-slate-950/80 p-6 shadow-glass backdrop-blur-xl sm:p-10">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {filtered.map((project, index) => (
          <motion.article key={project.title} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, delay: index * 0.08 }} className="glass rounded-[32px] border border-white/10 p-6 shadow-glass">
